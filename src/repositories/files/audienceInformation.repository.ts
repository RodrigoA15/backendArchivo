...
export const getAllAudienceInformation = async (numero_comparendo: string) => {
  const audienceInformation = await AppDataSource.createQueryBuilder()
    .select(
      `DISTINCT (AU.NRO_RADICADO_COMPARENDO), TI.COD_SIMIT, CP.FECHA FECHA_COMPARENDO,  TEC.DESCRIPCION_ESTADO, CP.FECHA_REGISTRA, CTR.DESCRIPCION ,CRP.NRO_RESOLUCION, CRP.FECHA_RESOLUCION,
